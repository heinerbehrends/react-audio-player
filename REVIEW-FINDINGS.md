...
from **337 jsdom / 51 E2E** to **504 jsdom / 112 E2E** — 56 specs on two
engines — all green, with `type-check` on all three TS projects, `lint`,
`prettier`, `build`, `check-exports` and `check-docs` clean throughout, and,
since **S19**, on React 18 and React 19 both.
...
| **F7**           | `onEnded` on `AudioPlayer` makes track-end observable, and userland playlists possible. The original row rejected an `ended` atom because the rewind cleared `el.ended` within a tick — that reasoning went with the rewind. Answered instead by `useIsAtEnd()`, a derivation: `onEnded` is the edge ("advance now"), `useIsAtEnd` the level ("the position is the end"), and they are different questions rather than competing answers                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
| **End-of-track** | The `ended` rewind is dropped: the element parks at the end, as `<audio>` and every streaming player do. **Measured in Chrome rather than argued**: `play()` on an ended element seeks to 0 on its own, so the rewind bought no replay and only destroyed the position a consumer's `onEnded` might read. Two things the probe corrected: `currentTime` parks _past_ `duration` (283.81 vs 283.33), so the proposed `areNumbersClose(currentTime, duration)` test would have read false at exactly the moment a track finished — `useIsAtEnd` uses `>=`; and a **paused** seek to `duration` leaves `el.ended` false, so the "seek-to-end semantics" worry does not arise. The `ended` handler now projects the position as well as `paused`, since the final `timeupdate` is not ordered against it. `Time.Remaining` drops its sign at zero, the resting state parking makes visible |
