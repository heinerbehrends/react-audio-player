...
| **F12**         | **A bug, and reachable more often than the finding says.** `getOffset` has no `range === 0` guard where its sibling `getProgress` does, so `maxValue === minValue` yields `(0 - 0) / 0` and the thumb gets `translate(calc(NaNpx - 50%), 0)` — invalid, so the browser drops the whole transform. Verified by running the arithmetic. That is a live stream permanently, and _every_ player before `loadedmetadata`, where `duration` is still 0. Cheapest real fix on this list.                                                                                                                                          |
...
| **T4**          | ~~`retries: 2` is unconditional~~ — **done**: `process.env.CI ? 2 : 0`, matching the idiom `forbidOnly` and `reuseExistingServer` already use. Note for whoever picks up **T7**: a `volume-state` row was observed failing and passing on retry during this work, and did not reproduce in 8 repeats. It is real and still unexplained; retries were what hid it.                                                                                                                           |
...
| **T7**          | 30+ fixed `waitForTimeout` calls as the only synchronisation. Worst: `toggle-play.spec.ts:27-37` waits 100 ms then reads `!audio.paused` — `play()` resolves asynchronously and decode start is unbounded, so it is a real race on a loaded CI box. It also does `await expect(isPlaying).toBe(true)` on a plain boolean, which does not retry. The suite already contains the right pattern in `ended.spec.ts` and `multi-instance.spec.ts`: `waitForFunction` with an explicit predicate. |
| **T8**          | 14 assertions pass a fractional `numDigits` to `toBeCloseTo(x, 0.25)`, giving a ±0.281 tolerance. Most are thin but sound and should simply be rewritten as `toBeLessThan(0.3)` so the number reads as what it is. The opposite problem in `drag-drop-time.spec.ts:78,84,86`: `toBeCloseTo(px, 1)` is ±0.05 px on a layout coordinate, a flake waiting to happen.                                                                                                                           |
| **T9**          | Two rate-button tests assert on the mock rather than the element, so they would pass if `handleMediaKeys` did nothing. The module-level `vi.spyOn` in `beforeEach` also disables the real hook for _every_ test in those files. `PlayButton.test.tsx` shows the correct shape against the fake.                                                                                                                                                                                             |
| **T6**          | The "grab-offset composition" block performs the subtraction under test inside the test, on two numbers derived from the same variable — `(60 + g) − g === 60` by construction. No production path decides anything, and its docstring claims an invariant it does not pin. The real coverage is elsewhere and is good.                                                                                                                                                                     |
| **T10**         | `handleSideEffects.test.ts:29-32` asserts `toBeUndefined()` on a function that returns `void` on every path. `createPlayerStore.test.ts` shows the right shape: `.not.toThrow()`.                                                                                                                                                                                                                                                                                                           |
| **T11**         | Change-detector tests: `calculateStyle.test.ts:163-191` asserts constants equal themselves, `:83-95` is a character-for-character duplicate of `:70-81`, and four "should export all subcomponents" rows assert `toBeDefined()` on statically-typed properties. Thin but harmless — only the duplicate is worth deleting.                                                                                                                                                                   |
