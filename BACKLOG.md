...
| **S17**     | ~~`PlaybackRate.Current` is declared `React.ReactElement                                                                                                           | null`but never returns`null`~~ — **done**. Kept the reserved space and corrected the type, and the span is now unconditional: a fragment in the current branch made the marker itself the flex item, so the row reflowed on every rate change — the reflow the hidden span exists to prevent. |
...
| Ref     | Item                                                                                                                                                                                                                                                                                                                                                                                   |
| ------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **S18** | ~~`react-dom` is an unnecessary peer dependency — zero references in the bundle~~ — **done**. Dropped from `peerDependencies`; it stays a devDependency, since the demo app mounts with it and the jsdom suite renders with it, and stays in tsup's `external` as a guard rather than a declaration.                                                                                   |
...
| **S19** | ~~React 19 is untested; devDeps pin 18 and there is no CI matrix, so `>=18.0.0` is unverified~~ — **done**. A `react-version: ["18", "19"]` matrix with `fail-fast: false` runs the whole gauntlet on both ends of the range; the 19 leg installs React, `react-dom` and both `@types` over the lockfile. One difference surfaced, and it was a test's ref callback, not library code. |
